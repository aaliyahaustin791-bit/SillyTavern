...
document.getElementById('call-end-btn')?.addEventListener('click', () => {
    ...
    const startArrivalWatcher = () => {
        ...
        arrivalObserver = new MutationObserver(async () => {
            ...
            const turnKey = last.getAttribute("mesid") || last.dataset?.mesid || last.id || last.textContent?.slice(0, 120);
            if (turnKey && turnKey === chatEl.dataset?.uieArrivalLastKey) return;
            ...
            const isUser =
                ...
                last.getAttribute("data-is-user") === "true";
            if (!isUser) {
                chatEl.dataset.uieArrivalLastKey = turnKey || "";
    ...
    const loadPhoneVisuals = () => {
        ...
        $("#uie-phone-custom-css").text(`
            ...
            .phone-app-icon{ border: 1px solid rgba(255,255,255,0.20); box-shadow: 0 1px 4px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.15); backdrop-filter: blur(4px); }
            .phone-brand{ color: #fff !important; text-shadow: 0 0 8px rgba(255,255,255,0.35), 0 1px 3px rgba(0,0,0,0.8) !important; }
        `);
...
${chat}`.slice(0, 6000), "System Check");
    ...
    $(document).off("click.phoneClose").on("click.phoneClose", "#uie-phone-close", () => {
        const $p = $("#uie-phone-window");
        const $b = $("#btn-phn");
        const wasVisible = $p.is(":visible");
        $p.fadeOut(200);
